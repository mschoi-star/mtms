// Sidebar = vertical strip of "program icons": rounded-square tile + label below.
// Designed to read like a classic OS desktop icon column.

const ProgramIcon = ({ id, name, glyph, active, onOpen }) => {
  return (
    <button
      onClick={() => onOpen(id)}
      style={{
        width: 92, padding: '10px 6px 8px', border: 'none', background: 'transparent',
    ...
    >
      ...
      <div style={{
        ...
        color:'#1f2430', textAlign:'center', lineHeight: 1.2,
...
// Glyphs use only allowed primitives: rectangles, circles, lines.
const Glyphs = {
  dashboard: (
...
const APPS = [
  { id:'dashboard', name:'Dashboard', glyph: Glyphs.dashboard },
  { id:'projects',  name:'Projects',  glyph: Glyphs.projects  },
  { id:'reports',   name:'Reports',   glyph: Glyphs.reports   },
  { id:'schedule',  name:'Schedule',  glyph: Glyphs.schedule  },
  { id:'inbox',     name:'Inbox',     glyph: Glyphs.inbox     },
  { id:'team',      name:'Team',      glyph: Glyphs.team      },
  { id:'files',     name:'Files',     glyph: Glyphs.files     },
  { id:'settings',  name:'Settings',  glyph: Glyphs.settings  },
];

const Sidebar = ({ openIds, onOpen }) => {
  return (
    <aside style={{
      width: 116, height:'100%', flexShrink:0,
      background:'#ffffff',
      ...
      {/* Sidebar header — looks like a window titlebar of the dock itself */}
      <div style={{
        height: 44, borderBottom:'1.5px solid #1f2430',
      ...
      <div style={{
        ...
        padding:'12px 6px 16px', display:'flex', flexDirection:'column', gap: 4,
        alignItems:'center'
      }}>
        {APPS.map(a => (
          <ProgramIcon
      ...
      <div style={{
        ...
        color:'#4a5160', display:'flex', justifyContent:'space-between'
      }}>
        <span>v2.6.1</span>
